...
def setup_case_ww3(ICE_RUNDIR, t0, tf):
  ...
  os.system('cp -f /nobackup/forsk/sm_malmu/CICE/oasis_ini/ww3_shel.inp {0}'.format(ICE_RUNDIR))
...
def setup_case_cice(ICE_RUNDIR, t0, deltat = 120.0, npt = 720, restart_fsd = '.false.'):
  ...
  if (not os.path.exists('{0}/{1}'.format(ICE_RUNDIR,fRestart_cice)) ):
    os.system('ln -sf /nobackup/forsk/sm_malmu/CICE/forcing/iced.2019-12-06-00000.nc {0}/{1}'.format(ICE_RUNDIR, fRestart_cice) )
...
def setup_case_oasis(ICE_RUNDIR, t0, tf):
  os.system('ln -sf /nobackup/forsk/sm_malmu/CICE/oasis_ini/rmp*.nc {0}'.format(ICE_RUNDIR) ) #remapping and grid files for oasis
